...
const MyListbox: FC<Dropdownprops> = ({ value, setValue, options }) => {
  // const [selectedValue, setSelectedValue] = useState(people[0]);

  return (
    <Listbox as="div" className="relative" value={value} onChange={setValue}>
      <Listbox.Label>Enter year</Listbox.Label>
      <Listbox.Button className=" px-4 py-2 rounded flex items-center gap-4 border-blue-500 border-2 w-full justify-between">
        {value.name}
        <svg
      ...
      <Listbox.Options className="my-1 py-2 border-2 border-red-700 flex gap-2 flex-col">
        {options.map((option) => (
          <Listbox.Option
            key={option.id}
            value={option}
